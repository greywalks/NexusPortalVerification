...
parts = listToArray(h.escapeHubText(seedish), "&lt;", true, true);
out = {
    ok:true,
    escape_seedish:h.escapeHubText(seedish),
    parts_count:arrayLen(parts),
    parts:parts,
    sanitized_dirty:h.sanitizeBulletinHtml(dirty),
    sanitized_seedish:h.sanitizeBulletinHtml(seedish),
    safe_tab:h.validLinkAddress("https://example.com/tab"),
    safe_rel:h.validLinkAddress("/training-tracker/"),
    safe_js:h.validLinkAddress("javascript:alert(1)"),
