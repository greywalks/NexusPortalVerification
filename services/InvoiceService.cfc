component output=false {
    ...
    struct function buildPhilips(required struct a,string invoiceTitle="TPV Philips Warehouse Invoice",string filename=""){
        var demo=nearest50(a.demo_additional_sqft);var service=nearest50(a.service_additional_sqft);var parts=nearest50(a.parts_sqft_manual);var subtotal=1910+demo*3.5+service*3.5+a.inbound_count*6+a.outbound_count*6+parts*3.5+a.repair_total+a.harvest_total;var tax=roundMoney(subtotal*.07);var total=roundMoney(subtotal+tax);
        var lines=[{description:"Warehouse Cost, First 500 sq. ft.",uom:"Each",unit_price:1910,quantity:1},{description:"Demo Additional Square Footage, in 50 sq ft. increments",uom:"Each",unit_price:3.5,quantity:demo},{description:"Service Additional Square Footage, in 50 sq ft. increments",uom:"Each",unit_price:3.5,quantity:service},{description:"Inbound Handling",uom:"Each",unit_price:6,quantity:a.inbound_count},{description:"Outbound Handling",uom:"Each",unit_price:6,quantity:a.outbound_count},{description:"Parts Additional Square Footage",uom:"Each",unit_price:3.5,quantity:parts},{description:"Repair",uom:"Unit Size/Each",unit_price:1,quantity:a.repair_total,total:a.repair_total},{description:"Harvest",uom:"Each",unit_price:1,quantity:a.harvest_total,total:a.harvest_total}];
        var name=safeFilename(arguments.filename,"TPV_Philips_Warehouse_Invoice")&".xlsx";var path=variables.outputPath&name;variables.workbooks.philips(path,a,arguments.invoiceTitle,{subtotal:subtotal,tax:tax,total:total});variables.outputs.register(name,"philips");
    ...
    struct function buildTCL(required struct a,required struct unitBreakdowns,required struct boxBreakdowns,string filename=""){
        ...
        var pallets=0;for(var k in valid.unit)pallets+=arrayLen(valid.unit[k]);var palletRate=pallets<=10?75:60;var tierCounts={"Serialized In Fee (1 part)":0,"2-5 Parts in Box":0,"6-10 Parts in Box":0,"11-15 Parts in Box":0,"16-20 Parts in Box":0};var tierRates={"Serialized In Fee (1 part)":3.85,"2-5 Parts in Box":3.85,"6-10 Parts in Box":7.70,"11-15 Parts in Box":11.55,"16-20 Parts in Box":15};var boxCount=0;
        ...
        var lines=[];if(pallets)arrayAppend(lines,{description:"FGI TV Inventory (in pallet)",uom:"Each",unit_price:palletRate,quantity:pallets});for(var label in tierCounts)if(tierCounts[label])arrayAppend(lines,{description:(left(label,10)=="Serialized"?"TV Part A-Grade Serialized Inventory (Single Part)":"TV Part A-Grade Inventory (#label#)"),uom:"Each",unit_price:tierRates[label],quantity:tierCounts[label]});var subtotal=0;for(var l in lines)subtotal+=l.unit_price*l.quantity;var tax=roundMoney(subtotal*.07);var total=roundMoney(subtotal+tax);var name=safeFilename(arguments.filename,"TCL_Warehouse_Invoice")&".xlsx";variables.workbooks.tcl(variables.outputPath&name,a,valid,lines,palletRate,{subtotal:subtotal,tax:tax,total:total});variables.outputs.register(name,"tcl");return {filename:name,total_pallets:pallets,pallet_rate:palletRate,box_count:boxCount,line_count:arrayLen(lines),subtotal:roundMoney(subtotal),tax:tax,total:total};
    }
    ...
    struct function buildWorkshop(required struct a,struct corrections={},string filename=""){
        var rows=[];var corrected=duplicate(a.rows);var unresolved={};for(var iss in a.issues)unresolved[iss.row_index&""]=true;
        for(var i=1;i<=arrayLen(corrected);i++){var r=corrected[i];var key=r.row_index&"";if(structKeyExists(arguments.corrections,key)){var corr=arguments.corrections[key];var field=isStruct(corr)?corr.field?:"":"";var valx=isStruct(corr)?corr.value?:"":corr;if(valx=="EXCLUDE"){r._exclude=true;structDelete(unresolved,key);}else if(field=="Derive Size"){r["Derive Size"]=valx;r._size=valx;r.Size=valx=="86"?"Large":"Small";structDelete(unresolved,key);}else if(field=="Category"){r.Category=valx;r._Type=deriveWorkshopType(valx);r.Type=r._Type;r._Type2=workshopType2(value(r,"Result"),valx);r.Type2=r._Type2;structDelete(unresolved,key);}else if(field=="Actual Serial"){r["Actual Serial"]=valx;var dm=serialModelSize(valx,variables.config.getSerialRules().rules?:[]);if(len(dm.model)){r["Actual Model"]=dm.model;r._clean_model=dm.model;}if(len(dm.size)){r._size=dm.size;r.Size=dm.size=="86"?"Large":"Small";}structDelete(unresolved,key);}}
            corrected[i]=r;if(!(r._exclude?:false)&&!structKeyExists(unresolved,key)&&listFindNoCase("55,65,70,75,86",r._size?:"")&&len(r._Type?:""))arrayAppend(rows,r);
        }
        var excluded=[];
