...
if(ap=="/inventory/quality/audit"&&method=="GET"){requireScope("inventory:read");qa=application.inventory.qualityAudit();if(!structCount(qa))apiError("not_ready","No Promethean reference is installed, so no audit is available.",409);apiOut({ok:true,audit:qa});}
