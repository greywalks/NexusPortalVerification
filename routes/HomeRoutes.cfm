...
function homeLinkHtml(required struct l){var label=encodeForHtml(l.label);var cls="home-link home-link-"&encodeForHtmlAttribute(l.style?:"normal");var url=len(l.url?:"")?l.url:"";var out="";
    ...
    else if((l.target?:"same")=="tab")out='<a class="'&cls&'" href="'&encodeForHtmlAttribute(url)&'" target="_blank" rel="noopener noreferrer">'&label&'</a>';
    else if((l.target?:"same")=="window")out='<a class="'&cls&'" href="'&encodeForHtmlAttribute(url)&'" target="_blank" rel="noopener noreferrer" data-open="window">'&label&'</a>';
    else out='<a class="'&cls&'" href="'&encodeForHtmlAttribute(url)&'">'&label&'</a>';
...
hp=removeChars(path,1,len("/home"));if(!len(hp))hp="/";
